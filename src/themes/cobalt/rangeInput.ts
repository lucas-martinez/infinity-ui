...
const { sliderButton, sliderHandle, sliderTrackOperated } = spec.global;
...
const extendThumb = css`
  ...
  border: ${rangeInput.thumb.borderWidth} solid ${pick(rangeInput.thumb.borderColor)};
...
const extendTrack = css`
  border-bottom: ${rangeInput.track.borderBottomWidth} solid ${rangeInput.track.borderBottomColor};
...
const generator: PartialThemeGenerator<'rangeInput'> = ({ baseSpacing = 24, scale = 6 }) => {
  return {
    rangeInput: {
      track: {
        ...rangeInput.track,
