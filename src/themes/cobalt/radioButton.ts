...
const { radio, radioLabel, radioTick } = spec.global;
...
export const radioButton = {
  ...
  innerWidth: spec.global.radioButtonSize,
  innerHeight: spec.global.radioButtonSize,
  ...
  tick: {
    ...
    left: spec.global.radioButtonControlSize,
    top: spec.global.radioButtonControlSize,
...
const baseStyle = css`
  ...
  div {
    ...
    height: ${radio.size};
    > svg {
      width: ${spec.global.radioButtonSize};
      height: ${spec.global.radioButtonSize};
    }
  }
...
const extend = css<ButtonProps>`
  ${baseStyle}
  ${props => !props.disabled && notDisabledStyle}
`;

const generator: PartialThemeGenerator<'radioButton'> = ({ baseSpacing = 24, scale = 6 }) => {
  return {
    radioButton: {
      ...radioButton,
      extend,
